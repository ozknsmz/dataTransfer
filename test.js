...
const url = `${
  process.env.BASEURL ? process.env.BASEURL : "file://" + process.cwd()
}${path}`;
...
describe("datachannel basic", () => {
  ...
  it("transfers a file", async () => {
    await driver
      .findElement(webdriver.By.id("fileInput"))
      .sendKeys(process.cwd() + "/src/content/devices/multi/images/poster.jpg");
    await driver.wait(() =>
      driver.findElement(webdriver.By.id("sendFile")).isEnabled()
    );
    ...
    await driver.wait(() =>
      driver.findElement(webdriver.By.id("download")).isEnabled()
    );
  });
});
